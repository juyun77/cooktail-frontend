...
import { updateCocktail, getCocktailById } from '../../api/Cocktail';
import { getToken } from '../../api/Auth';

const CocktailUpdatePage = () => {
  ...
  useEffect(() => {
    const fetchCocktailData = async () => {
      try {
        ...
        const cocktail = await getCocktailById(id, token); // 칵테일 데이터 가져오기
        setCocktailData(cocktail); // 가져온 칵테일 데이터로 상태 업데이트
      } catch (error) {
        console.error('칵테일 데이터 가져오기 오류:', error.message);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = await getToken(); // 토큰 가져오기
      await updateCocktail(id, cocktailData, token); // 칵테일 데이터 수정
      navigate(`/cocktails/${id}`); // 수정된 칵테일 페이지로 이동
    } catch (error) {
...
export default CocktailUpdatePage;
